...
ORDER_BOOK_SCHEDULE = 8.0  # executed each X seconds
...
TWITTER_BEARER = env.str("TWITTER_BEARER")
...
for _, x in EXCHANGES_AND_PAIRS_OF_REFERENCE.items():
    assert (
        x["pair"] in COIN_OR_TOKEN_REFERENCE["VAR"] + COIN_OR_TOKEN_REFERENCE["STABLE"]
    )
