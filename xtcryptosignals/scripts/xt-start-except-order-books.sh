...
nohup bash -c "FLASK_ENV=production SETTINGS_APP=`pwd`/config/client.prod.env xt-client --num-workers 4" > /dev/null 2>&1 &
...
nohup bash -c "redis-cli FLUSHALL && SETTINGS_APP=`pwd`/config/server.prod.env xt-tasks -t cfgi -t project -t tether -t notifications -t ticker --log-ticker-minimal" > /dev/null 2>&1 &
